...
- **Windows:** a single `make.exe` targeting the mingw-w64 runtime. One Windows-only source call (`src/job.c`, Windows batch-file creation) trips `-Werror=format-security`; it's demoted to a warning — the argument is `map_windows32_error_to_string`'s output, not attacker input.
